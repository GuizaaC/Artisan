...
if (isset($_FILES['img_usuario']) && $_FILES['img_usuario']['error'] === UPLOAD_ERR_OK) {
    $targetDir = '...imagens';  // Substitua pelo caminho real do diretório
    $targetFile = $targetDir . basename($_FILES['img_usuario']['name']);
    $imageFileType = strtolower(pathinfo($targetFile, PATHINFO_EXTENSION));

    // Exibe informações para depuração
    echo "Nome do arquivo: " . $_FILES['img_usuario']['name'] . "<br>";
    echo "Tipo do arquivo: " . $_FILES['img_usuario']['type'] . "<br>";
    echo "Tamanho do arquivo: " . $_FILES['img_usuario']['size'] . "<br>";
    echo "Caminho temporário: " . $_FILES['img_usuario']['tmp_name'] . "<br>";
    echo "Erro no upload: " . $_FILES['img_usuario']['error'] . "<br>";
    ...
    if (in_array($imageFileType, $allowedExtensions)) {
        ...
        if (move_uploaded_file($_FILES['img_usuario']['tmp_name'], $targetFile)) {
            ...
            $query = "INSERT INTO usuario (email_usuario, nome_usuario, endereco_usuario, telefone_usuario, cpf_usuario, data_nascimento_usuario, senha_usuario, caminho_imagem)
                      VALUES ('".$_POST["email_usuario"]."', '".$_POST["nome_usuario"]."', '".$_POST["endereco_usuario"]."', '".$_POST["telefone_usuario"]."', 
                      '".$_POST["cpf_usuario"]."', '".$_POST["data_nascimento_usuario"]."', '".$senhaCriptografada."', '".$caminhoImagem."');";
            ...
            // Exibe mensagem de erro de upload
            echo "Erro ao fazer upload do arquivo.";
        }
    } else {
        // Exibe mensagem de erro de tipo de arquivo
        echo "Apenas arquivos JPG, JPEG e PNG são permitidos.";
    }
} else {
    // Exibe mensagem quando nenhum arquivo de imagem foi enviado
